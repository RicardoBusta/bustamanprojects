...
RayTracingWidget::RayTracingWidget(Scene &scene,QWidget *parent):
  QWidget(parent),
  scene(scene),
  moving_(false),
...
void RayTracingWidget::mouseMoveEvent(QMouseEvent *event)
{
  ...
  last_pos = event->pos();
  scene.rotx += float(delta.y())/2.0;
  scene.roty += float(delta.x())/2.0;
  //updateFast();
  emit Changed();
}
...
{
  scene.distz+=float(event->delta())/100.0f;
  emit Changed();
}
...
//  if(!scene.valid_) return;
  ...
  for(int i=0;i<auto_update_steps;i++){
    ...
    if( scene.track_one_ray && (auto_update_i==scene.specialx || auto_update_j==scene.specialy) && !(auto_update_i==scene.specialx && auto_update_j==scene.specialy)){
    ...
    Ray ray(scene.frustum[0],far_plane.Normalized(),far_plane.Length(),near_plane.Length(),Ric::Color(0xff000000));
    ray.calc(ray.cast(scene),scene,1,scene.calculate_advanced_light);
...
//  if(!scene.valid_) return;
  ...
  for(int i=0;i<image_.width();i++){
    ...
    for(int j=0;j<image_.height();j++){
      if( scene.track_one_ray && (i==scene.specialx || j==scene.specialy) && !(i==scene.specialx && j==scene.specialy)){
      ...
      Ray ray(scene.frustum[0],far_plane.Normalized(),far_plane.Length(),near_plane.Length(),Ric::Color(0xff000000));
      const TriangleFace *f = ray.cast(scene);
      ray.calc(f,scene,0,false);
